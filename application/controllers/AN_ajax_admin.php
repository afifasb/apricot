...
class AN_ajax_admin extends CI_Controller
{
	...
	function submit_artikel($i=0){
		...
		$kirim_artikel=($this->input->post('kirim_artikel'));
		...
		if($i==0){
			...
			$data=array('artikel_judul'=>$judul,
						...
						'artikel_kirim_api'=>$kirim_artikel
			);
			...
			if ($kirim_artikel == 1) {
				$this->load->model('admin/api_connect','api_connect');

				$private_key ='jfkdaso&^^78932789HKDShudohusdia52719';
				$url = "https://sibangkoman.pu.go.id/center/pelatihan/api/index.php";

				$action = 'insert_berita';
				...
				$result = $this->api_connect->trf_send_request($url, $private_key, $params, $action);
			}
