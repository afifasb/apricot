...
class Galeri
{
    ...
    function galeri_perid(){

		$data=$this->CI->db->query("SELECT galeri_id,galeri_nama,
        LEFT(galeri_deskripsi, 300) AS galeri_deskripsi FROM `galeri`  
        ORDER BY `galeri`.`galeri_id` DESC
		");
	...
	function semua_galeri($limit,$offset){
        ...
        AND user.status_user='Y' AND user.terhapus='N' 
        AND galeri.galeri_user=user.id_user AND galeri.kategori_id=kategori_galeri.id 
        ORDER BY galeri.galeri_id DESC 
        -- LIMIT $offset,$limit
		");
